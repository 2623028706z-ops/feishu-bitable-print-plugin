...
`npm run start` 会先生成生产构建，再通过 `PORT` 环境变量（默认 3000）启动公开服务。将服务地址添加到飞书多维表格的“扩展脚本”；脱离飞书打开时会展示脱敏样例数据。
...
插件内可组合筛选客户、品类、花束。订单出货日期支持单日、明确的开始/结束区间和全部日期；默认筛选今天。`T+n` 只调整标签上显示的日期，不改变订单筛选结果，例如订单出货日为 2026-08-23、标签偏移为 2 天时，标签显示 2026-08-25。标签数量可按销售数量或设置为每单固定张数；“养护说明”字段可在标签内容中单独开启。
