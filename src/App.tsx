...
import { adjustPrintDate, defaultLabelConfig, defaultPrintFilter, expandLabelCopies, filterOrders, formatSelectedDateRange, groupOrdersForWorkOrders, issueLabel, labelPrintPageMetrics, labelPrintPageStyle, sampleOrders, splitCategoryValues, type LabelConfig, type PrintFilter, type PrintOrder } from './lib/print-model';
...
export default function App() {
  ...
  return <div className={`app-shell ${mode}`}>
    <aside className="control-panel">
      ...
      <div className="panel-footer"><IssueSummary orders={filteredOrders} mode={mode} />{error && <div className="error-text">{error}</div>}<button className="primary-button" disabled={!activeCount || !printValid} onClick={() => print()}><Printer size={17} />打印 {activeCount} {mode === 'label' ? '张标签' : '张加工单'}</button></div>
    </aside>
