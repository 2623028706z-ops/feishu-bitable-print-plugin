...
export default function App() {
  ...
  return <div className={`app-shell ${mode}`}>
    <aside className="control-panel">
      ...
      <div className="panel-footer"><IssueSummary orders={filteredOrders} />{templateMessage && <div className="info-text">{templateMessage}</div>}{error && <div className="error-text">{error}</div>}{mode === 'label' ? <button className="secondary-button" onClick={saveSharedLabelTemplate}>保存为共享标签模板</button> : <button className="secondary-button" onClick={saveSharedWorkTemplate}>保存为共享加工单模板</button>}<button className="primary-button" disabled={!activeCount || !printCheck.valid} onClick={() => print()}><Printer size={17} />打印 {activeCount} {mode === 'label' ? '张标签' : '款加工单'}</button><button className="secondary-button" onClick={() => setMode(mode === 'label' ? 'work-order' : 'label')}><ChevronDown size={16} />切换到{mode === 'label' ? '加工单' : '标签'}</button></div>
    </aside>
