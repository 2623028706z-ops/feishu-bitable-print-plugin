...
import { adjustPrintDate, defaultLabelConfig, defaultPrintFilter, expandLabelCopies, filterOrders, formatSelectedDateRange, groupOrdersForWorkOrders, issueLabel, labelPrintLayout, labelPrintPageStyle, normalizeLabelPrinterFeed, sampleOrders, splitCategoryValues, type LabelConfig, type LabelPrinterFeed, type PrintFilter, type PrintOrder } from './lib/print-model';
...
import { createDefaultTemplate, migrateTemplateConfig, resizeLabelTemplateForPaper, type LabelTemplate } from './domain/templates';
...
function readLabelConfigMemory(): LabelConfig {
  let raw: Partial<LabelConfig> = {};
  try { raw = JSON.parse(storageGet(CONFIG_STORAGE_KEY) || '{}') as Partial<LabelConfig>; } catch { /* Ignore malformed local settings. */ }
  const stored = { ...defaultLabelConfig, ...raw };
  const printerFeed = normalizeLabelPrinterFeed({ width: stored.width, height: stored.height, printerFeed: raw.printerFeed, printRotation: raw.printRotation });
  const isLegacySheetDefault = stored.columns === 2 && stored.rows === 8 && stored.gapX === 2 && stored.gapY === 2 && stored.marginX === 5 && stored.marginY === 5;
  const normalized = { ...stored, printerFeed, printRotation: 0 as const };
...
function IssueSummary({ orders, mode, columns }: { orders: PrintOrder[]; mode: Mode; columns: string[] }) {
  ...
  const codeMissing = orders.some((order) => order.issues.includes('missing-code'));
  if (!issues.length) return <div className="clean-status"><Check size={15} /> 数据完整，可生成打印预览</div>;
  return <div className="issue-box"><AlertTriangle size={16} /><div><strong>{issues.length} 项数据需要确认</strong><div>{issues.slice(0, 3).join('；')}{issues.length > 3 ? '…' : ''}</div>{codeMissing && columns.length > 0 && <div className="issue-columns">读不到花束编码。当前表字段：{columns.join(' · ')}。请把编码列改名成「成品编码」，或告诉我它现在叫什么。</div>}</div></div>;
}
...
function LabelSheet({ orders, config, template }: { orders: PrintOrder[]; config: LabelConfig; template?: LabelTemplate }) {
  ...
  return <>{Array.from({ length: pageCount }, (_, pageIndex) => <div className={`print-surface label-sheet${layout.rotation ? ' label-sheet-rotated' : ''}${pageIndex < pageCount - 1 ? ' label-page-break' : ''}`} style={style} key={`label-page-${pageIndex}`}>
    {labels.slice(pageIndex, pageIndex + 1).map((order) => <div className="label-card" key={order.recordId} style={{
      position: layout.rotation ? 'absolute' : 'relative',
      left: 0,
      top: 0,
      transformOrigin: 'top left',
      transform: layout.transform,
      WebkitTransformOrigin: 'top left',
      WebkitTransform: layout.transform,
    }}>
...
export default function App() {
  ...
  const updateLabelSize = useCallback((patch: Partial<Pick<LabelConfig, 'width' | 'height'>>) => {
    const width = patch.width ?? config.width;
    const height = patch.height ?? config.height;
    setLabelTemplate((current) => resizeLabelTemplateForPaper(current, width, height));
    updateConfig({ width, height, printerFeed: width >= height ? 'landscape' : 'portrait', printRotation: 0 });
  }, [config.width, config.height, updateConfig]);
  const setLabelOrientation = useCallback((orientation: 'landscape' | 'portrait') => {
    ...
    const next = wantsLandscape === isLandscape
      ? { ...config, printerFeed: orientation, printRotation: 0 as const }
      : { ...config, width: config.height, height: config.width, printerFeed: orientation, printRotation: 0 as const };
  ...
  const resetLabelLayout = useCallback(() => {
    const restored = resizeLabelTemplateForPaper(createDefaultTemplate('label'), config.width, config.height);
    setLabelTemplate(templateFromConfig(config, restored));
  }, [config]);
  ...
  const labelPrintGeometry = useMemo(() => labelPrintLayout(config), [config]);
  ...
  return <><style media="print">{printPageStyle}</style><div className={`app-shell ${mode}`}>
    <aside className="control-panel">
      ...
      {mode === 'label' ? <div className="panel-section"><div className="section-title"><span>标签规格</span><Button className="reset-filter" theme="borderless" type="tertiary" size="small" onClick={resetLabelLayout}>恢复默认排版</Button><Settings2 size={15} /></div><div className="field-grid"><NumberField label="宽 mm" value={config.width} onChange={(value) => updateLabelSize({ width: Math.min(300, Math.max(5, value)) })} /><NumberField label="高 mm" value={config.height} onChange={(value) => updateLabelSize({ height: Math.min(300, Math.max(5, value)) })} /><NumberField label="列数" value={config.columns} onChange={(value) => updateConfig({ columns: Math.min(20, Math.max(1, value)) })} /><NumberField label="行数" value={config.rows} onChange={(value) => updateConfig({ rows: Math.min(20, Math.max(1, value)) })} /></div><div className="orientation-control"><span>标签版式</span><RadioGroup className="segmented" type="button" value={config.width >= config.height ? 'landscape' : 'portrait'} onChange={(event) => setLabelOrientation(event.target.value as 'landscape' | 'portrait')} aria-label="标签版式"><Radio value="landscape">横向</Radio><Radio value="portrait">纵向</Radio></RadioGroup></div><div className="orientation-control"><span>输出纸张方向</span><RadioGroup className="segmented" type="button" value={config.printerFeed} onChange={(event) => updateConfig({ printerFeed: event.target.value as LabelPrinterFeed, printRotation: 0 })} aria-label="输出纸张方向"><Radio value="landscape">横向 {Math.max(config.width, config.height)}×{Math.min(config.width, config.height)}</Radio><Radio value="portrait">纵向 {Math.min(config.width, config.height)}×{Math.max(config.width, config.height)}</Radio></RadioGroup></div><div className="field-grid"><NumberField label="横间距" value={config.gapX} onChange={(value) => updateConfig({ gapX: Math.min(50, Math.max(0, value)) })} /><NumberField label="纵间距" value={config.gapY} onChange={(value) => updateConfig({ gapY: Math.min(50, Math.max(0, value)) })} /></div><div className="field-grid"><NumberField label="内边距 mm" value={config.padding} onChange={(value) => updateConfig({ padding: Math.min(20, Math.max(0, value)) })} /><NumberField label="内容间距 mm" value={config.contentGap} onChange={(value) => updateConfig({ contentGap: Math.min(20, Math.max(0, value)) })} /></div></div> : <div className="panel-section"><div className="section-title"><span>加工单模板</span><LayoutGrid size={15} /></div><WorkOrderEditor value={workTemplate} onChange={setWorkTemplate} region={workRegion} onSelectRegion={setWorkRegion} /></div>}
