...
export function migrateTemplateConfig(raw: unknown, type: TemplateType): PrintTemplate {
  ...
  result.fixedCopies = Math.min(500, Math.max(1, Math.round(numberValue(source.fixedCopies, 1, 1))));
  ...
  result.elements = result.elements.map((element: any) => clampLabelElementToBounds({ ...element, kind: element.kind === 'care' ? 'care' : element.kind, width: numberValue(element.width, 1, .5), height: numberValue(element.height, 1, .5), fontSize: numberValue(element.fontSize ?? element.fontSizeMm, result.styles.fontSize, 1), fontSizeMm: numberValue(element.fontSizeMm ?? element.fontSize, result.styles.fontSize, 1), align: element.align ?? element.textAlign ?? result.styles.align }, result));
  return result;
}
